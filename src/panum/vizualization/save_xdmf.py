...
class SaveXDMFCallback:
    def __init__(
        ...
        """
        arguments:
        - file_paths: a dictionary where the keys represent the
            component to be saved, and the values are the paths
        ...
        for component, path in file_paths.items():
            ...
            self.output_files.update(
                {component: XDMFFile(MPI.COMM_WORLD, path, "w")}
            )
            self.output_files[component].write_mesh(msh)
            self.output_files[component].write_function(
                functions[component], t0
            )
